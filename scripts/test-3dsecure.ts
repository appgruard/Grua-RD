 ...
 * 
 * Uso: npx tsx scripts/test-3dsecure.ts
 */
...
async function makeRequest(url: string, data: any, auth1: string, auth2: string): Promise<any> {
...
async function step1_InitialPaymentRequest(
  card: typeof TEST_CARDS.frictionlessWithMethod,
  ...
  const baseUrl = 'https://app.gruard.com'; // Cambiar por tu URL real

  const request: Step1Request = {
...
async function step2_ProcessThreeDSMethod(
  azulOrderId: string,
  status: 'RECEIVED' | 'EXPECTED_BUT_NOT_RECEIVED' | 'NOT_EXPECTED'
): Promise<AzulResponse> {
  console.log('\n' + '='.repeat(80));
...
async function step3_ProcessChallengeResult(
  azulOrderId: string,
  cRes: string
): Promise<AzulResponse> {
  console.log('\n' + '='.repeat(80));
  ...
  // Usar el mismo endpoint para procesar el resultado del desafío
  const response = await makeRequest(
    AZUL_TEST_URL,
    request,
    TEST_CONFIG.auth1,
...
async function runFullTest(cardKey: keyof typeof TEST_CARDS) {
  ...
  try {
    ...
    if (step1Response.IsoCode === '3D2METHOD') {
      ...
      if (step2Response.IsoCode === '3D' && step2Response.ThreeDSChallenge) {
        ...
        // En un flujo real, aquí el usuario completaría el desafío y recibiríamos el cRes
...
async function main() {
  ...
  const testCard = process.argv[2] as keyof typeof TEST_CARDS || 'frictionlessWithMethod';
